...
public class Leetcode621 {
    public static void main(String[] args) {
        ...
        System.out.println(leastInterval(task1, 8));
    }
    ...
    public static int leastInterval(char[] tasks, int n) {
        ...
        for (int i = 0; i < n + 1 && !maxHeap.isEmpty(); i++) {
            if (cache[i] == 0) cache[i] = maxHeap.poll();
            while (cache[i] < cache[0] - 1 && !maxHeap.isEmpty()) {
                if (cache[i] + maxHeap.peek() < cache[0] - 1) cache[i] += maxHeap.poll();
                else {
                    if (i + 1 < n + 1)
                        cache[i + 1] = maxHeap.poll() - (cache[0] - 1 - cache[i]);
                    cache[i] = cache[0] - 1;
                }
            }
        }
        int idle = 0;
        for (int i = 1; i < n + 1; i++) {
            if (cache[i] < cache[0] - 1) idle += cache[0] - 1 - cache[i];
        }
